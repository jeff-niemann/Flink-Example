...
@Slf4j
public class SegmenterSolution extends KeyedProcessFunction<String, Event, Segment> {
    ...
    public void processElement(Event value, Context ctx, Collector<Segment> out) throws Exception {
        ...
        if ("FOCUS".equals(value.getEventType()) && !value.getTarget().equals(lastEventValue.getTarget())) {
            ...
            newSegment.setTarget(lastEventValue.getTarget());
            newSegment.setEvents(StreamSupport.stream(eventListState.get().spliterator(), false).collect(
                Collectors.toList()));

            out.collect(newSegment);
    ...
    private void updateTimer(Context ctx) throws IOException {
        ...
        // Remove the current timer if it exists
        // TODO - this could have some range so we don't need to clear and set the timer for every event
        var timer = timerState.value();
        if (timer != null) {
            ctx.timerService().deleteProcessingTimeTimer(timer);
        }

        // Register the new timer
        long time = ctx.timerService().currentProcessingTime() + SESSION_CLOSE_MS;
        ctx.timerService().registerProcessingTimeTimer(time);

        // Track the timer time so we can delete it later
